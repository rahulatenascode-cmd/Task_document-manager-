...
def _ensure_schema():
    ...
    This helper checks for the new fields (`is_deleted`, `deleted_at`,
    `changed_by`, `reason`) that were introduced during the hardening
    task. If they are absent, ALTER TABLE statements are executed to
    add them. Using a full migration tool like Alembic is recommended
    for production but this simple routine eases development.
    """
